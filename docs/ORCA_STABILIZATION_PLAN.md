...
5. Keep `./gradlew testDebugUnitTest`, `./gradlew assembleDebug`, `git diff --check`, `scripts/verify_android.sh object-process <device>`, `scripts/verify_android.sh orca-object-label-parity <device>`, `scripts/verify_android.sh orca-import-smoke <device>`, and `scripts/verify_android.sh orca-3mf-roundtrip-device <device>` green before pushing to a device.
