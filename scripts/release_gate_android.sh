...
MODE="${MOBILE_SLICER_RELEASE_GATE_MODE:-full}"
...
Usage:
  ...
  scripts/release_gate_android.sh --public [serial]
...
non-debug local/release audits without requiring debug automation to be enabled
in the public build.
...
case "$MODE" in
  full|public)
    ;;
  *)
    fail "Unknown release gate mode: $MODE"
    ;;
esac
