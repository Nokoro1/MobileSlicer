...
ARTIFACT_ROOT="${MOBILE_SLICER_ORCA_RICH_PROJECT_ARTIFACT_ROOT:-$ROOT_DIR/artifacts/orca-project-fixtures}"
REFERENCE_DIR="${MOBILE_SLICER_ORCA_RICH_PROJECT_REFERENCE_DIR:-$ROOT_DIR/regression-fixtures/orca-project-references/rich-object-settings}"
STAMP="$(date +%Y%m%d-%H%M%S)"
RUN_DIR="$ARTIFACT_ROOT/rich-object-settings-$STAMP"
ORCA_LIB_SHIM="$ROOT_DIR/tools/orcaslicer/lib-shim/usr/lib64:$ROOT_DIR/tools/orcaslicer/lib-shim"

usage() {
  cat <<'USAGE'
Usage:
  scripts/generate_orca_rich_project_fixture.sh

Generates a desktop-Orca 3MF reference package with:
  - two plates,
  - two named objects,
  - two object-to-filament assignments,
  - object-scoped print setting metadata,
  - project settings/config entries,
  - Orca package thumbnails.

This intentionally uses Orca's CLI load_assemble_list path. It does not claim
modifier-volume parity, because the local Orca CLI does not expose an official
parameter-modifier creation path.
Height-range settings are also intentionally excluded from this sliced package
fixture because the local desktop Orca CLI segfaults before writing diagnostics
when --slice and assemble-list height_ranges are combined. Project-only export
accepts that input, but this fixture gates sliced .gcode.3mf package structure.

Environment:
  ORCA_SLICER_BIN
      Orca binary to run. Defaults to vendor/orcaslicer/build/package/bin/orca-slicer.
  MOBILE_SLICER_ORCA_RICH_PROJECT_ARTIFACT_ROOT
      Artifact root for generated inputs, logs, and output package.
  MOBILE_SLICER_ORCA_RICH_PROJECT_REFERENCE_DIR
      Checked reference directory populated after the audit passes.
USAGE
}

log() {
  printf '[generate_orca_rich_project_fixture] %s\n' "$*"
}

fail() {
  printf '[generate_orca_rich_project_fixture] ERROR: %s\n' "$*" >&2
  exit 1
}
...
for index, (path, color) in enumerate(zip(filament_paths, ("#F2754E", "#4EA3F2")), start=1):
    ...
    filament["filament_settings_id"] = [f"MobileSlicer rich project fixture filament {index}"]
...
PACKAGE_NAME="rich-object-settings.gcode.3mf"
...
log "Generated rich Orca project fixture: $REFERENCE_DIR"
